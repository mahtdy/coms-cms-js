...
import { SystemErrorLog } from "./fileManager";
...
const proxyUrl: string = 'YOUR_PROXY_URL';
const proxyPort: string = 'YOUR_PROXY_PORT';
const proxyUsername: string = 'YOUR_PROXY_USERNAME';
const proxyPassword: string = 'YOUR_PROXY_PASSWORD';


export default class SeoServices {
    constructor() {
    ...
    static async getPageSpeed(url: string, device: string): Promise<any> {
        ...
        //     throw error
        // }
        const apiUrl: string = 'https://www.googleapis.com/pagespeedonline/v5/runPagespeed';
    }
    ...
    static async getWebMasterInfo(credential: any,
        token: any
        , domain: string,
        startDate: string = "2023-12-13",
        endDate: string = "2024-03-06"): Promise<any> {
    ...
    static async getWebMasterDomains(
        credential: any,
        token: any
    ) {
        var oAuth2Client = new googleapis.google.auth.OAuth2(
    ...
    static async getWebMasterAllInfo(credential: any,
        token: any, domain: string,
        startDate: string = "2021-04-03", endDate: string = "2021-07-05"): Promise<any> {
        var oAuth2Client = new googleapis.google.auth.OAuth2(
            {
                redirectUri: credential.web.redirect_uris[0],
                clientId: credential.web.client_id,
    ...
    static async saveWebMasterDataToDB(credential: any,
        token: any, domain: string, startDate: string) {
        ...
        while (fromDate < new Date()) {
            ...
            try {
                ...
                while (startRow == 1 || (dataList && dataList.rows)) {
                    console.log(domain, this.getValidDateForWM(fromDate),
                        this.getValidDateForWM(toDate),)
                    var dataList = await this.getWebMasterInfoByDimension(credential,
                        token, domain, [
                        // "query",
                        "page",
                        // "country",
    ...
    static async getWebMasterInfoByDimension(credential: any,
        token: any, domain: string,
        dimensions: string[], startDate: string = "2021-04-03", endDate: string = "2021-07-05", startRow: number = 0): Promise<any> {
        var oAuth2Client = new googleapis.google.auth.OAuth2(
            {
                redirectUri: credential.web.redirect_uris[0],
                clientId: credential.web.client_id,
    ...
    static listNotIndexedUrls(credential: any,
        token: any) {
    ...
    static async indexUrl(credential: any,
        token: any, url: string): Promise<any> {
    ...
    static async getUrlMetaData(credential: any,
        token: any, url: string): Promise<any> {
        var oAuth2Client = new googleapis.google.auth.OAuth2(
            {
                redirectUri: credential.web.redirect_uris[0],
    ...
    static getValidDateForWM(date: Date) {
